...
import { not, and, merge } from "../operators/operations.ts";
...
import { bgRed, bgGreen, red, green, blue, yellow, bold, white, bgBlue, cyan, magenta } from "https://deno.land/std/fmt/colors.ts";
...
function printValidationResults(result: ValidationResult): void {
    ...
    if (result.reasons.length > 0) {
        ...
        result.reasons.forEach((reason, index) => {
            ...
            console.log(`- ${bold("Permission key")}: ${magenta(reason.permissionKey)}`);
            console.log(`- ${bold("Message")}: ${red(reason.message)}`);
            if (reason.stateIndex !== undefined) {
                console.log(`- ${bold("State index")}: ${yellow(reason.stateIndex.toString())}`);
            }
        });
    } else {
...
// Success case
printTestHeader(1, "Success case - user requesting to view an authorized group");
...
printTestHeader(2, "Failure case - user trying to delete themselves (denySelf)");
