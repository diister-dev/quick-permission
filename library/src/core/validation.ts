 ...
 * @module validation
 */
import type { PermissionHierarchy, PermissionKey, PermissionRequests, PermissionStateSet, ValidationError, ValidationResult } from "../types/common.ts";
import { satisfiedBy } from "./hierarchy.ts";
 ...
 */
function allow(schemas: any[], rules: any[], state: any, request: any, permKey: string) {
 ...
 */
function mergeValidationResults(results: { valid?: boolean; errors: ValidationError[] }[], mode: "and" | "or" = "and"): { valid?: boolean; errors: ValidationError[] } {
...
>(hierarchy: H, states: S, key: K, request: R): ValidationResult {
    ...
    for (let stateIndex = 0; stateIndex < states.length; stateIndex++) {
        ...
        for (const permKey of satisfier) {
            ...
            const result = allow(permission.schemas, permission.rules, permissionState, request, permKey);
