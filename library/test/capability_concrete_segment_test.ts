...
Deno.test({
  name: "cap-mode auto-fetch — concrete segment matching resource.id is fetched + evaluated",
  ...
  const r1 = await sys.context({ subject: { id: "user:lucas" } }).can(
    "participants.list",
    ["exposition:beta-owned", "participant:*"],
  );
  assertEquals(r1.ok, true, "Beta's expo should pass");
  assertEquals(fetchCount > 0, true, "expoOf should have been fetched in cap mode");
  ...
  const r2 = await sys.context({ subject: { id: "user:lucas" } }).can(
    "participants.list",
    ["exposition:globex-owned", "participant:*"],
  );
  assertEquals(r2.ok, false, "Globex's expo should deny — Beta admin not authorized");
});

Deno.test({
  name: "cap-mode auto-fetch — full wildcard target keeps legacy silent-pass",
  sanitizeResources: false,
  sanitizeOps: false,
}, async () => {
  let fetchCount = 0;
...
Deno.test({
  name: "cap-mode auto-fetch — 3-seg perm with mixed concrete/wildcard fetches each independently",
  sanitizeResources: false,
  sanitizeOps: false,
}, async () => {
  interface OrgDoc {
  ...
  const r = await sys.context({ subject: { id: "user:lucas" } }).can(
    "expo.orgs.roles.read",
    ["exposition:beta-owned", "org:exhibitor-1", "role:*"],
  );
  assertEquals(r.ok, true);
...
Deno.test({
  name: "cap-mode auto-fetch — denies when one resource fails its check",
  sanitizeResources: false,
  sanitizeOps: false,
}, async () => {
  interface OrgDoc {
  ...
  const r = await sys.context({ subject: { id: "user:lucas" } }).can(
    "expo.orgs.read",
    ["exposition:beta-owned", "org:sponsor-1"],
  );
  assertEquals(r.ok, false);
});

Deno.test({
  name: "cap-mode auto-fetch — resource.id with no matching segment → legacy silent-pass",
  sanitizeResources: false,
  sanitizeOps: false,
}, async () => {
  ...
  assertEquals(r.ok, true);
  assertEquals(fetchCount, 0, "no fetch — id matches no segment, legacy preserved");
  if (r.ok) {
